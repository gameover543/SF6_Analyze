...
export function buildKnowledgeContext(
  ...
  // 引用ルール
  if (lines.length > 0) {
    lines.push(`### 知識の使い方（重要）`);
    lines.push(
      `- **ナレッジに書かれた技名・性能は必ずフレームデータで裏取りすること**。ナレッジは動画からAIが抽出したもので、技名の取り違えや性能の誤記が含まれることがある`
    );
    lines.push(
      `- 例:「OD流酔拳で弾抜け」とナレッジにあっても、フレームデータでOD流酔拳に飛び道具無敵がなければ、別の技（OD爆廻等）の可能性を検討すること`
    );
    lines.push(
