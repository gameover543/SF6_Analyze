...
### 確反の基本
- ガード後に相手の硬直差の絶対値以下の発生を持つ技で確反できる。相手が-6Fなら発生6F以下の技で確反
- 相手が-4Fなら発生4Fの技で確反可能。-3Fは確反なし（最速が4Fのため）
- **ただしフレーム上は間に合っても、ガード後の距離（ヒットバック）で技が届かない場合がある。特に先端ガード時は確反が届かないケースが多い。確反の話をする際は距離の条件にも触れること**
- 一般的な確反始動：弱P（4-5F）、中P（6-8F）、強攻撃（確反が大きい時）
- 投げは発生5F。-5以上ある技には投げ確反が可能（密着時）
